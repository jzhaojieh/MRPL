...
classdef cubicSpiralTrajectory < handle
    %cubicSpiralTrajectory Implements a planar trajectory specified in terms of three 
    ...
    methods(Static = true)
        ...
        function makeLookupTable(scale)
            % Make 6 lookup tables to be used for cubicSpiralTrajectory generation.
    ...
    methods(Access = public)
        ...
        function obj = cubicSpiralTrajectory(parms,numSamples)
            % Constructs a cubicSpiralTrajectory for the supplied parameters. The
       ...
       function planVelocities(obj,Vmax)
        ...
        function J = getJacobian(obj)
            eps = 0.00001;
            dp = [eps 0.0 0.0];
            clothPlus1 = cubicSpiralTrajectory(obj.parms+dp,obj.numSamples);
            ...
            dp = [0.0 0.0 eps];
            clothPlus3 = cubicSpiralTrajectory(obj.parms+dp,obj.numSamples);            
        ...
        function refineParameters(obj,goalPose)
            ...
            while(error > 0.001)
                ...
                disp(error);
                if(n >=20)
                    fprintf('cubicSpiralTrajectory failing to refine Parameters\n');
                    break;
                end
            end
        end
